"""
score_predictor.py (full rewrite)

Goal:
- Train a personalized score predictor (your rating) from your AniList list + AniList dataset
- Train a "will I finish it?" model that is robust for newer shows by using:
    - True DROPPED as negative
    - COMPLETED as positive
    - CURRENT/PAUSED/ON_HOLD inferred as "effectively dropped" only if stale,
      using your own completion pace learned from COMPLETED entries + episode count

Outputs:
- anilist_personalized_predictions_full.csv

Inputs:
- my_anilist_list.csv              (your exported list; must contain 'AniList ID', 'Status', 'Score', 'Progress', dates)
- anilist_anime_data_complete.csv  (AniList full dataset)

Notes:
- Uses genres + tags (from AniList dataset JSON columns)
- Uses HistGradientBoosting for both score + finish models
- Uses tqdm progress bar for prediction batches
"""
import json
import numpy as np
import pandas as pd
...
FINISH_SPLIT_MODE = "random"
TEST_FRACTION = 0.20
HOLDOUT_YEAR = 2025
SCORE_SPLIT_MODE = "year_holdout"
FINISH_SPLIT_MODE = "random"
...
def get_preprocessed_feature_names(prep: ColumnTransformer, X_sample: pd.DataFrame):
    """
    Returns feature names after ColumnTransformer transforms.
    Works with the specific ('cat', OneHotEncoder, cat_cols) / ('num', StandardScaler, numeric_features) pattern.
    """
    names = []
...
def export_permutation_importance(
    ...
    """
    Compute and export permutation feature importance for a fitted model or wrapper.

    This function measures the importance of each original input column by repeatedly
    permuting that column and observing the drop in model performance. Results are
    written to CSV and the top features are printed.

    Args:
        pipeline: Fitted estimator supporting sklearn-style prediction.
        X: Evaluation features in their original, pre-encoded dataframe form.
        y: Evaluation target aligned with `X`.
        out_csv: Path to the CSV file where importances will be saved.
        top_k: Number of top features to print to stdout.
        title: Optional label used in console output.
        n_repeats: Number of shuffle repeats per feature.
        max_rows: Maximum number of rows to use; larger inputs are sampled.
        random_state: Random seed for row sampling and permutations.
        scoring: Optional sklearn scoring string.

    Returns:
        A DataFrame sorted by descending mean permutation importance.

    Notes:
        - Importance is measured on the original feature columns, not one-hot-expanded
          columns, which makes the output easier to interpret.
        - This method is model-agnostic and works even when impurity-based feature
          importances are unavailable.
    """
    if len(X) > max_rows:
...
def export_feature_importance(pipeline: Pipeline, X_sample: pd.DataFrame, out_csv: str, top_k: int = 50, title: str = ""):
    """
    Exports impurity-based feature importances for HistGradientBoosting models inside a Pipeline.
    """
    ...
    if not hasattr(prep, "transformers_"):
        raise RuntimeError("Preprocessor is not fitted yet. Fit the pipeline before exporting importances.")

    feat_names = get_preprocessed_feature_names(prep, X_sample)
...
def build_days_per_episode_by_bucket(completed_df: pd.DataFrame) -> dict:
    """
    Use your COMPLETED entries (with StartDT + CompletedDT + episodes) to estimate your pace:
    median days/episode for episode-count buckets.
    """
    ...
    for b in med:
        med[b] = float(np.clip(med[b], 0.8, 20.0))

    if "very_long" in med and "long" in med:
        med["very_long"] = max(med["very_long"], med["long"])   
...
def infer_finish_label(
    ...
    """
    Infer whether an entry should be treated as finished, dropped, or unresolved.

    The labeling logic is designed for training the finish model. Explicit
    COMPLETED entries are treated as positive examples, explicit DROPPED entries
    as negative examples, and some CURRENT/PAUSED/ON_HOLD entries are inferred
    as effectively dropped if they appear stale relative to the user's historical
    completion pace.

    Args:
        row: A dataframe row containing user list status, progress, episode count,
            anime airing status, and start date.
        today: Reference date used to measure staleness.
        dpe_by_bucket: Mapping of episode-count bucket to estimated user pace in
            days per episode, learned from completed entries.
        min_days_floor: Minimum number of days before a show can be considered stale.
        multiplier: Multiplier applied to the expected completion time to define the
            stale cutoff.
        min_progress_fraction: Minimum watched fraction below which very early
            entries are protected from being labeled as dropped too aggressively.

    Returns:
        1 for finished,
        0 for dropped or inferred effectively dropped,
        None when the status should remain unresolved and excluded from finish training.

    Notes:
        - PLANNING entries are always unresolved.
        - CURRENT entries for still-releasing or not-yet-released shows are not
          inferred as dropped.
        - If progress is at least the episode count, the entry is treated as finished
          even if the status is not marked COMPLETED.
    """
...
def report_single_anime_score(
    ...
    """
    Print a SHAP-based explanation for the score prediction of one anime.

    The report shows the crowd baseline score, the model's predicted residual
    relative to crowd score, the final predicted personal score, and the strongest
    positive and negative feature contributions.

    Args:
        anime_id: AniList anime ID to explain.
        anilist_valid: Filtered AniList dataframe aligned with the score feature matrix.
        anilist_pos: Mapping from AniList ID to row position in `anilist_valid`.
        X_all_score: Full score feature matrix before preprocessing.
        prep_score: Fitted preprocessor used by the score model.
        xgb_score: Fitted XGBoost regressor predicting score residuals.
        shap_values: SHAP values aligned to the transformed score matrix.
        score_feature_names: Feature names for the transformed matrix.
        resid_bias: Bias correction applied to residual predictions.
        mean_score_10_all: Crowd mean score on the 1–10 scale aligned with `anilist_valid`.
        top_k: Number of top contributing features to display.

    Returns:
        None. This function prints a formatted report.

    Notes:
        - The score model predicts personal residual relative to the crowd mean score.
        - Final predicted score is reconstructed as crowd score plus corrected residual.
        - This helper is intended for interpretability and manual inspection.
    """
...
def fit_multihot_rate_encoding(X: pd.DataFrame, y: np.ndarray, cols: list[str], smooth: float = 20.0):
    """
    y is 0/1 (e.g., drop label).
    Returns smoothed per-column rate toward global mean.
    """
...
def fit_multihot_resid_encoding(X: pd.DataFrame, y: np.ndarray, cols: list[str], smooth: float = 10.0):
    """
    X[cols] is 0/1.
    y is residual target (0–10 scale).
    Returns arrays aligned to cols: mean_resid_per_col, count_per_col, global_mean
    Uses simple smoothing toward global mean.
    """
    M = X[cols].to_numpy(dtype=np.float32)
    y = np.asarray(y, dtype=np.float32)
...
def apply_multihot_resid_features(
    ...
    """
    Adds dense summary features to X:
      {prefix}_mean_resid
      {prefix}_mean_support
      {prefix}_present
    """
...
def build_features(df_in: pd.DataFrame, include_list_years: bool, use_stats: bool, stats_mode: str = "finish"):
    """
    Build the base feature matrix used by the score and finish models.

    This function normalizes raw AniList fields, creates derived numeric features,
    encodes selected categorical metadata, and optionally adds community-distribution
    statistics parsed from AniList JSON columns.

    Args:
        df_in: Input dataframe containing AniList metadata and, when available,
            user list-derived fields.
        include_list_years: Whether to include list-specific year features such as
            start and completion year from the user's history.
        use_stats: Whether to parse and include community status/score distribution
            features from AniList dataset columns.
        stats_mode: Controls which stats-derived features are kept. Expected values
            are "score" or "finish".

    Returns:
        A tuple of:
            - X_base: DataFrame containing engineered features
            - cat_cols: list of categorical column names intended for one-hot encoding

    Notes:
        - Community context features are controlled globally by USE_COMMUNITY_CONTEXT.
        - Missing numeric fields are coerced to 0.
        - Missing categorical fields are filled with "UNKNOWN".
        - Multilabel genre/tag indicators are added outside this function.
    """
    df = df_in.copy()
...
def fit_finish_xgb_with_early_stopping(
    ...
    """
    Train the finish classifier with an internal validation split and early stopping.

    This function fits a fresh preprocessing pipeline on the training fold, transforms
    both training and validation data, and trains an XGBoost binary classifier using
    early stopping on validation performance.

    Args:
        X_train_df: Training features as a pandas DataFrame.
        y_train: Binary training targets where 1 indicates drop-risk and 0 indicates
            completion.
        w_train: Per-row sample weights used to rebalance inferred vs. true drop labels
            and class imbalance.
        prep_finish: Unfitted ColumnTransformer describing preprocessing for the finish
            model.
        random_state: Random seed used for the train/validation split and model.
        test_size: Fraction of the provided training data reserved for validation.
        early_rounds: Number of rounds without improvement before stopping.

    Returns:
        An object exposing `predict_proba` and `predict` that applies preprocessing
        before calling the fitted XGBoost model.

    Notes:
        - Stratified splitting is used when both classes are present.
        - The returned wrapper keeps the fitted preprocessor and model together so
          downstream code can operate on raw feature DataFrames.
    """
...
def time_based_split_indices(dates: pd.Series, test_fraction: float = 0.20):
    """
    Returns (train_idx, test_idx) as numpy arrays of original row indices.
    Sorts by date and takes the most recent fraction as test.
    """
...
def main():
    ...
    w_drop[finish_train["is_inferred_drop"].to_numpy(bool)] = INFERRED_DROP_WEIGHT
    w_drop[finish_train["is_true_dropped"].to_numpy(bool)] = TRUE_DROP_WEIGHT

    X_all_base_score, _ = build_features(anilist_valid, False, True, "score")
    X_all_base_finish, _ = build_features(anilist_valid, False, True, "finish")
    ...
    X_finish     = X_finish_base.reset_index(drop=True).copy()
    X_all_finish = X_all_base_finish.reset_index(drop=True).copy()
    ...
    if SCORE_SPLIT_MODE == "year_holdout":
        score_year = pd.to_datetime(rated_scored["CompletedDT"], errors="coerce").dt.year
        score_year = score_year.fillna(pd.to_datetime(rated_scored["StartDT"], errors="coerce").dt.year)
        score_year = score_year.fillna(0).astype(int).reset_index(drop=True)

        tr_mask = (score_year < HOLDOUT_YEAR).to_numpy()
        te_mask = (score_year == HOLDOUT_YEAR).to_numpy()

        Xs_tr = X_score.iloc[np.flatnonzero(tr_mask)].copy()
        Xs_te = X_score.iloc[np.flatnonzero(te_mask)].copy()
        ys_tr = y_score[tr_mask]
        ys_te = y_score[te_mask]
    elif SCORE_SPLIT_MODE == "time":
    ...
    if FINISH_SPLIT_MODE == "year_holdout":
        rel_year = pd.to_numeric(finish_train["startDate_year"], errors="coerce").fillna(0).astype(int)
        rel_year = rel_year.reset_index(drop=True)

        tr_mask = (rel_year < HOLDOUT_YEAR).to_numpy()
        te_mask = (rel_year == HOLDOUT_YEAR).to_numpy()

        Xf_tr = X_finish.iloc[np.flatnonzero(tr_mask)].copy()
        Xf_te = X_finish.iloc[np.flatnonzero(te_mask)].copy()
        yf_tr = y_drop[tr_mask]
        yf_te = y_drop[te_mask]
        wf_tr = w_drop[tr_mask]
        wf_te = w_drop[te_mask]
    elif FINISH_SPLIT_MODE == "time":
    ...
    dense_candidates_finish = [
        ...
        "pred_user_score10",
        "episodes", "duration",
        "stats_log1p_n_total", "stats_p_completed", "stats_p_dropped", "stats_retention", "stats_hype_gap",
        "stats_p90plus", "stats_p50minus",
    ]
    ...
    cal_X = oof_proba.astype(np.float32).reshape(-1, 1)

    calibrator.fit(cal_X, yf_tr, sample_weight=sample_weight)

    def calibrated_proba(base_probs: np.ndarray) -> np.ndarray:
        Xc = np.asarray(base_probs, dtype=np.float32).reshape(-1, 1)
    ...
    proba_drop = calibrated_proba(base_te).astype(np.float32)
    ...
    from sklearn.metrics import recall_score, confusion_matrix, classification_report, balanced_accuracy_score
    ...
    for t in ths:
        ...
        if r_drop > best_drop_recall_oof:
            ...
            best_t_safe_oof = t
    best_t_finish_safe = best_t_safe_oof
    ...
    class _FinishPredictor:
        ...
        def predict_proba(self, X: pd.DataFrame):
            ...
            Xc = base.reshape(-1, 1).astype(np.float32)
            cal = self.calibrator_lr.predict_proba(Xc)[:, 1]

            return np.vstack([1.0 - cal, cal]).T

    finish_model = _FinishPredictor(finish_base_full, calibrator_full, [])
    ...
    base_drop = finish_model.predict_proba(X_all_finish)[:, 1].astype(np.float32)
    base_finish = 1.0 - base_drop
    ...
    for j, col in enumerate(tqdm(finish_explain_cols, desc="Finish ablation (by col)")):
        ...
        Xp[col] = 0
        drop2 = finish_model.predict_proba(Xp)[:, 1].astype(np.float32)
        finish2 = 1.0 - drop2
        deltas[:, j] = base_finish - finish2
